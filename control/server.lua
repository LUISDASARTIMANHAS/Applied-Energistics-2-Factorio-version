...
local function NetworkCreationGui(info)
    ...
    guis.addCloseButtonTitleBar(newgui, {"entity-name.server-block"})

    local contentFrame = newgui.add({
        type = "frame",
        name = "network-ui-content-frame",
        style = "inside_shallow_frame_with_padding"
    })

    local content = contentFrame.add({
        type = "flow",
        name = "network-ui-content",
        direction = "vertical",
        style = "content-flow"
    })
    ...
    local entityPreviewFrame = content.add({
        type = "frame",
        name = "network-ui-entity-preview-frame",
        style = "deep_frame_in_shallow_frame"
    })
    local entityPreview = entityPreviewFrame.add({
        type = "entity-preview",
        name = "network-ui-entity-preview",
    ...
    networkNameFlow.add({
        ...
        caption = {"gui.network-name-selection"}
    })

    local networkNameSelectFlow = content.add({
        type = "flow",
        name = "network-ui-netname-select-flow",
        direction = "horizontal"
    })

    local networkNameField = networkNameSelectFlow.add({
        type = "textfield",
        name = "network-name-selector",
    ...
    networkNameSelectFlow.add({
        type = "sprite-button",
        name = "network-name-selector-confirm",
...
util.OverwriteGui("server-block", NetworkCreationGui, GuiRedrawHandlerFactory)

util.AddEventHandler({defines.events.on_built_entity, defines.events.on_robot_built_entity},
    function(e)
        if e.created_entity.name == "server-block" then
...
util.AddEventHandler({defines.events.on_entity_died, defines.events.on_player_mined_entity, defines.events.on_robot_mined_entity},
    function(e)
        if e.entity.name == "server-block" then
...
util.AddEventHandler(defines.events.on_gui_click,
    function(e)
        if e.element.name == "network-name-selector-confirm" and e.button == defines.mouse_button_type.left then
            global.networks[guiControl.currentGui.entity.unit_number].name = e.element.parent["network-name-selector"].text
        end
    end
)